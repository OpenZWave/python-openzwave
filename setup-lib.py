...
if '--dev' in sys.argv:
    ...
    index = sys.argv.index('--embed')
    sys.argv.pop(index)
    template = EmbedTemplate()
elif '--shared' in sys.argv:
    index = sys.argv.index('--shared')
    sys.argv.pop(index)
    ...
    def finalize_options(self):
        if self.openzwave_dir is None:
            if getattr(self, 'develop', False) or not getattr(self, 'install', False):
                self.openzwave_dir = template.openzwave
            else:
                build = self.distribution.get_command_obj('build')
                build.ensure_finalized()
                self.openzwave_dir = os.path.join(build.build_lib, template.openzwave)
...
setup(
  ...
  cmdclass = {'build_ext': template.build_ext,
        'bdist_egg': bdist_egg,
        'build': build,
